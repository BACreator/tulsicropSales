...
//Get all the selected product info from local storage
    function getProductFromStorage(){
    	...
    	for (var i = 0; i < newprod.length; i++) {
    		...
    		html+='<td>'+ '<input style="width: 50%; height: auto; text-align: center;" type="text" value="'+newprod[i].proQuant+'" />' +'</td>';
            //html+='<td onclick="">'+ '<input type="text" value="" />'+newprod[i].proQuant+'</td>';
            /*html+='<td>'+
            '<div class="p_qty"><span>Add Quantity</span><div class="value-button" id="decrease" onclick="decreaseValue()" value="Decrease Value">-</div><input type="number" name="qty" id="number" value="" /><div class="value-button" id="increase" onclick="increaseValue()" value="Increase Value">+</div></div>'
                        +'</td>';*/
